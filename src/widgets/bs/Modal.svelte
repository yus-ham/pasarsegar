...
<script>
  ...
  $: classes = `${dialogBaseClass} ${className}`
             + (size ?` modal-${size}`:``)
             + (centered ?` ${dialogBaseClass}-centered`:``)
             + (scrollable ?` ${dialogBaseClass}-scrollable`:``)
</script>
  ...
  <div
    ...
    {#if isOpen}
      <div
        ...
        class="{`modal show `+ modalClassName}"
        role="dialog"
        style="display: block;"
      ...
      <div
        ...
        class="{`modal-backdrop show `+ backdropClassName}" />
    {/if}
  </div>
{/if}
